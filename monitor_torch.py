"""Torch-based audio monitor that streams classification probabilities over MQTT."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Monitor mic input using a PyTorch checkpoint")
    parser.add_argument("--model", type=Path, required=True, help="Path to a trained PyTorch checkpoint (.pt)")
    parser.add_argument("--mqtt-topic", type=str, required=True, help="MQTT topic to publish JSON payloads")
    ...
    parser.add_argument(
        ...
        default=None,
        help="Optional torch device override (defaults to auto-detect)",
    )
    parser.add_argument(
        "--img-size",
        type=int,
...
def resolve_device(preferred: str | None) -> torch.device:
    ...
    if getattr(torch, "mps", None) and torch.mps.is_available():  # type: ignore[attr-defined]
